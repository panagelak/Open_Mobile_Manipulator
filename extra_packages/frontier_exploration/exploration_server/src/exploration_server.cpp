...
ExplorationServer::ExplorationServer(ros::NodeHandle nh,
                                     ros::NodeHandle private_nh)
    : nh_(nh), private_nh_("~"), retry_(5), tf_listener_(),
      tf2_listener_(tf2_buffer_), success_(false), moving_(false),
      move_client_("/move_base", true),
      previous_state_(actionlib::SimpleClientGoalState::PENDING),
      explore_action_server_(
          nh, "exploration_server_node",
          boost::bind(&ExplorationServer::goalCb, this, _1),
          boost::bind(&ExplorationServer::cancelGoalCb, this, _1), false) {
  costmap_ros_ = boost::make_shared<costmap_2d::Costmap2DROS>("explore_costmap",
                                                              tf2_buffer_);
...
void ExplorationServer::goalCb(GoalHandle gh) {
  ...
  ros::ServiceClient polygon_client =
      private_nh_.serviceClient<exploration_msgs::SetPolygon>(
...
void ExplorationServer::moveBaseResultCb(
    const actionlib::SimpleClientGoalState &state,
    const move_base_msgs::MoveBaseResultConstPtr &result) {
  previous_state_ = state;
  planner_->addToVisited(move_client_goal_.target_pose.pose.position,
                         previous_state_);
  if (state == actionlib::SimpleClientGoalState::ABORTED) {
    if (move_client_goal_.target_pose.pose.position.x ==
            explore_center_.point.x &&
        move_client_goal_.target_pose.pose.position.y ==
            explore_center_.point.y) {
      ROS_ERROR(
          "Failed to move to exploration center point. Exploration Failed.");
      return;
    } else {
  ...
  // regardless of whether the robot was successful in reaching the point or
  // not, request the next goal from the plugin and send it to move_base
  if (ExplorationServer::inBoundary()) {
    ExplorationServer::requestAndSendGoal();
  }
}
...
bool ExplorationServer::inBoundary() {
  ...
  // evaluate if robot is within exploration boundary using robot_pose in
  // boundary frame
  geometry_msgs::PoseStamped eval_pose = pose1;
  if (eval_pose.header.frame_id != polygon_.header.frame_id) {
    try {
      tf_listener_.waitForTransform(polygon_.header.frame_id,
                                    pose1.header.frame_id, pose1.header.stamp,
                                    ros::Duration(.1));
  ...
  // check if robot is not within exploration boundary and needs to return to
  // center of search area
  if (polygon_.polygon.points.size() > 0 &&
      !pointInPolygon(eval_pose.pose.position, polygon_.polygon)) {
    // check if robot has explored at least one point, and promote debug message
    // to warning
    if (success_) {
    ...
    if (eval_point.header.frame_id != explore_center_.header.frame_id) {
      ...
      tf_listener_.waitForTransform(explore_center_.header.frame_id,
                                    temp.header.frame_id, temp.header.stamp,
                                    ros::Duration(.1));
      tf_listener_.transformPoint(explore_center_.header.frame_id, temp,
                                  eval_point);
    }
...
void ExplorationServer::requestAndSendGoal() {
  ...
  point_list = planner_->whereToExplore(
      current_pose, move_client_goal_.target_pose.pose.position,
      previous_state_);
  ...
  if (point_list.size() != 0) {
    ...
    { // scope the mutex lock
      ...
      move_client_.sendGoal(
          move_client_goal_,
          boost::bind(&ExplorationServer::moveBaseResultCb, this, _1, _2), 0,
          0);
      moving_ = true;
      lock.unlock();
    }
    ...
    // try to get a goal from the planner 5 times but throw an error on the 5th
    // time
    if (retry_ == 0) {
