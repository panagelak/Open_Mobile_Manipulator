...
void callback(my_dyn_rec::lidartfConfig &config, uint32_t level) {
  ...
  transform1.setOrigin( tf::Vector3(config.camera_px, config.camera_py, config.camera_pz) );
  tf::Quaternion q1;
  q1.setRPY(config.camera_or, config.camera_op, config.camera_oy);
  transform1.setRotation(q1);
  br2.sendTransform(tf::StampedTransform(transform, ros::Time::now(), "kinect_base2", "kinect_dummy"));
}
